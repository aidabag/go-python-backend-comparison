...
Нагрузочное тестирование проводится с помощью инструмента **[k6](https://k6.io/)**. Все сценарии используют единый профиль нагрузки — трёхступенчатую модель с нарастанием числа виртуальных пользователей (VU):
...
> Сервисы используют один и тот же порт 8080, поэтому одновременно может работать только один из них.
...
### 4. Генерация аналитических отчётов

```powershell
# Установка зависимостей для анализа
pip install polars matplotlib numpy

# Запуск анализа для всех сценариев в Power Shell
