...
function Run-ApiTests {
    ...
    Write-Host "`n--- ШАГ 1: ПРОВЕРКА СОСТОЯНИЯ (HEALTH CHECK, PYTHON) ---" -ForegroundColor Cyan
    Invoke-WebRequest -Uri "$BaseUrl/health" -UseBasicParsing

    Write-Host "`n--- ШАГ 2: МЕТРИКИ (METRICS, PYTHON) ---" -ForegroundColor Cyan
    ...
    Write-Host "`n--- ШАГ 6: СОЗДАНИЕ ЗАКАЗА 1 ---" -ForegroundColor Cyan
    ...
    Write-Host "`n--- ШАГ 7: ПРОВЕРКА ОСТАТКОВ (ТОВАР 1) ---" -ForegroundColor Cyan
    Invoke-RestMethod -Uri "$BaseUrl/products/$p1Id"

    Write-Host "`n--- ШАГ 8: СОЗДАНИЕ ЗАКАЗА 2 ---" -ForegroundColor Cyan
    ...
    Write-Host "`n--- ШАГ 9: ПРОВЕРКА ОСТАТКОВ (ТОВАР 2) ---" -ForegroundColor Cyan
    Invoke-RestMethod -Uri "$BaseUrl/products/$p2Id"

    Write-Host "`n--- ШАГ 10: СОЗДАНИЕ ЗАКАЗА 3 ---" -ForegroundColor Cyan
...
while ($true) {
    try {
        if ((Invoke-RestMethod "http://localhost:8080/health").status -eq "ok") { break }
    } catch {}
